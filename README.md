...
```json
{
  "mcpServers": {
    "career-agent": {
      "command": "uvx",
      "args": ["--from", "git+https://github.com/yogurt-c/career-agent-mcp@v0.1.0", "job-agent-mcp"]
    }
  }
}
```
...
```bash
claude mcp add career-agent -- uvx --from git+https://github.com/yogurt-c/career-agent-mcp@v0.1.0 job-agent-mcp
```
...
새 버전이 릴리즈되면 README의 버전 번호(`@vX.X.X`)가 바뀝니다.

**Claude Desktop**

`claude_desktop_config.json`에서 버전 번호를 새 버전으로 수정 후 Claude Desktop 재시작:

```json
"args": ["--from", "git+https://github.com/yogurt-c/career-agent-mcp@v0.1.1", "job-agent-mcp"]
```
...
claude mcp add career-agent -- uvx --from git+https://github.com/yogurt-c/career-agent-mcp@v0.1.1 job-agent-mcp
```
